...
6. **Provenance manifest — `scripts/build_manifest.py`**  
   The manifest records the verified source hash, Python/Matplotlib versions and SHA-256 hashes of every expected code, evidence and derived-product file. It also checks the PNG evidence structure and chunk checksums, and fails rather than silently recording a missing, truncated or corrupt expected file.
