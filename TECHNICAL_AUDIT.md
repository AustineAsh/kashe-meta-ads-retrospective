...
| **Data validation** | Strong internal consistency checks | Schema, sequential row IDs, privacy scan, numeric/range checks, reach/impression relationship, flag consistency and cost-per-result reconciliation. All 122 recognised rows reproduce Meta's exported cost per result within `1e-6`. | Internal consistency cannot independently validate Meta's historical measurement or attribution system. |
...
| **Testing** | Good portfolio-level coverage | Unit tests cover XLSX coordinates and redaction; dataset tests cover quality flags and headline KPIs; integration tests rerun the public pipeline; with the private source present, preparation must recreate the committed CSV byte-for-byte. | Tests are focused on the known export and published claims rather than exhaustive property-based or cross-platform testing. |
| **Code readability/consistency** | Strengthened | PEP 8-oriented layout plus Ruff lint checks in CI. | Passing automated style checks is not evidence that the analytical logic is correct; logic is checked separately through tests and source reconciliation. |
| **Dependency capture** | Good, not fully hermetic | Matplotlib and Ruff versions are pinned, CI declares Python 3.13, and the run manifest records Python/Matplotlib versions and hashes of code/products. | The complete execution environment is not containerised or locked to a Python patch version and OS image, so this is reproducible rather than bit-for-bit environment preservation. |
| **Provenance** | Strong | Original source SHA-256, public-derivative hash, transformation summary and deterministic run manifest link source, code and outputs. | The restricted source cannot be independently downloaded from the public repository. |
...
| **Deterministic public outputs** | Strong | SVG metadata is stabilised; the public pipeline regenerates the same downstream products and CI uses `git diff --exit-code` to detect drift. | Matplotlib/runtime changes outside the pinned dependency and declared Python line could still affect future render details if the environment changes. |
...
The public CI now compiles the modules, runs tests, lints code with Ruff, rebuilds all public products and fails if regenerated outputs differ from the committed files. GitHub's current Python Actions guidance recommends `setup-python` for consistent runner behaviour and provides Ruff as an example lint/format step.

## Overall judgement

For a portfolio-scale retrospective analysis, the pipeline is now **end-to-end reproducible from the restricted source in a local authorised environment and reproducible from the sanitised public dataset in GitHub Actions**. The strongest features are preservation of source rows, explicit handling of uncertainty, internal arithmetic validation, separation of unlike result types, automated regeneration and provenance hashes.
